...
def cal_rmse_main(dataset = 'yama', threhods = '0.95', image_last_name = "bmp"):
    ...
    save_ref.to_csv('./match_results/' + dataset + threhods + str(epoch) + '_matched_ref_points.csv', header = None)
    save_sen.to_csv('./match_results/' + dataset + threhods + str(epoch) + '_matched_sen_points.csv', header = None)
    ...
    cv2.imwrite('./visual_result/' + dataset + threhods + str(epoch) + '_matched_poI1.bmp', combine_img)

    print('The Rmse per eopch is:',rmse_list)
    print('The Rmseloo per eopch is:',rmse_loo_list)
