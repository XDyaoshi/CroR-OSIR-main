...
resnet = tmodel.resnet18(pretrained = False)
backbone = nn.Sequential(*list(resnet.children())[:-1])#Removed the final linear head
def weights_init(m):
...
class simCNN_contrastive(nn.Module):
    def __init__(self, classid_list, num_ABN=5, head='mlp', feature_dim=128):
        ...
        self.backbone = backbone #Use networks like ResNet-50 or ResNet-18 as backbone

   
        self.maxpool = nn.AdaptiveMaxPool2d((4, 4))# maxpool is used

        if head == 'linear':   # Access the linear layer for classification
            self.head = nn.utils.spectral_norm(nn.Linear(self.feature_dim, self.logit_dim))# Spectral normalization is used
        elif head == 'mlp':    # Access mlp for comparison
            self.head = nn.Sequential(
                nn.utils.spectral_norm(nn.Linear(self.feature_dim*64, self.feature_dim*8)), 
                nn.ReLU(inplace=True),
                nn.utils.spectral_norm(nn.Linear(self.feature_dim*8, self.logit_dim))
            )

        self.apply(weights_init) # Initial weight
        self.to(self.device)
    ...
    def forward(self, x):
        features = self.get_feature(x) #[512,2048]
...
class LinearClassifier(nn.Module):
    ...
    def get_logprob(self, logits, temperature=1):
        ...
        return probs # This is the value of the probability distribution
    ...
    def get_mix_loss(self, logits_R, logits_S, targets, temperature=1, begin_index=0, end_index=4499, branch='R'):
        ...
        target_indices = torch.where(targets == 1)[1]  
    ...
    def get_cross_loss(self, logits_R, logits_S, targets, mask_martix_R, mask_martix_S, temperature=1, begin_index=0, end_index=4499, branch='R'):
        ...
        if branch == 'R':
            ...
            for idx in class_S_index:
                mask = (pesudo_label[:, 1] - 1) == target_indices[idx]
                # Find the real label that the pseudo-label is in, and assign it the value at the corresponding position of the pseudo-label
                mask_martix[idx, pesudo_label[mask][:, 0].int() - 1] = (probs[mask]>=0.1).float()
            ...
            for idx in class_R_index:
                mask = (pesudo_label[:, 1] - 1) == target_indices[idx]
                mask_martix[idx, pesudo_label[mask][:, 0].int() - begin_index - 1] = (probs[mask]>=0.1).float()
    ...
    def estimate_threshold_logits(self, feature_encoder, validation_loader, percentile=5):
        ...
        for i, (images, labels) in enumerate(validation_loader):
            ...
            with torch.no_grad():
                ...
                maxLogit, maxIndexes = torch.max(logits, 1) # The label with the largest probability is taken as the predicted value
    ...
    def predict_logits_pre(self, features, threhods):
        logits = self.forward(features)
        probs = self.get_prob(logits = logits,temperature = 1)
