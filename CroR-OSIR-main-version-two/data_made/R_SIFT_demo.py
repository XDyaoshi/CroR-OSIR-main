...
def run_demo(data_name,image_name,dir_path,Max_keypionts = 500,Distance = 20, flag = 0):
    ...
    for kp in keypoints:
        x, y = kp.pt
        if x > edge_threshold and y > edge_threshold and x < image.shape[1] - edge_threshold and y < image.shape[0] - edge_threshold:
            deled_keypoints.append(kp)

    filtered_keypoints = []
    for kp1 in deled_keypoints:
        is_valid = True
    ...
    print("The num of keypoints is:", len(filtered_keypoints))
