...
def Cross_rejective_main(dataset = 'yama', threhods = 0.99, batch_size = 1024, lr = 0.00002, num_classifier_epochs = 11,
                         label_smoothing_coeff = 0, feature_dim = 128*16, begin_epoch = 5, check_epoch = 5, image_last_name = 'bmp'):
    ...
    train_loader_classifier, _ = load_ImageNet200([train_root], category_indexs=classid_training, train=True, batchSize=batch_size, useRandAugment=True)
    validation_loader_R, _ = load_ImageNet200([train_root], category_indexs=classid_R, train=False, batchSize=batch_size, useRandAugment=False)
    validation_loader_S, _ = load_ImageNet200([train_root], category_indexs=classid_S, train=False, batchSize=batch_size, useRandAugment=False)
    test_loader_R, _ = load_ImageNet200([test_R_root], category_indexs = classid_S, train = False, batchSize=batch_size, useRandAugment=False)
    test_loader_S, _ = load_ImageNet200([test_S_root], category_indexs = classid_R, train = False, batchSize=batch_size, useRandAugment=False)
    ...
    for classifier_epoch in range(num_classifier_epochs):
        ...
        if(classifier_epoch % check_epoch == 0 and classifier_epoch != 0):
            ...
            # if(classifier_epoch % 5 == 0):
                # T_i = calculate_T.cal_T_main(dataset = dataset, threhods = threhods, epoch = classifier_epoch, begin_epoch = begin_epoch)
                
            R_mse, R_loo = calculate_rmse.cal_rmse_main(dataset = dataset, threhods = str(threhods), image_last_name = image_last_name, begin_epoch = 5, end_epoch = classifier_epoch + 1, indent = 5)
            R_mse_list.append(R_mse)
            R_loo_list.append(R_loo)
            # if(classifier_epoch % 5 == 0):
                # pretrain_made.made_data(data_name = dataset, image_last_name = image_last_name, T0 = T_i)
    ...
    print('The Rmse per iter is:', R_mse_list)
    print('The Rmse_loo per iter is:', R_loo_list)