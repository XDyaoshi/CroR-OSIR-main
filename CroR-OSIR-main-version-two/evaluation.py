...
def openset_eval_contrastive_logits(model, classifier, known_test_loader, unknown_test_loader):
    ...
    for images, labels in known_test_loader:
        ...
        prediction, logits = classifier.predict_logits(features) # These two steps are forward propagation
        for i in range(len(images)):
            total_known += 1
            if prediction[i] == labels[i]:
                correct_known += 1 # One more correct match pointpairs
    ...
    for images, labels in unknown_test_loader:
        ...
        features = model.get_trm_feature(images)
        prediction, logits = classifier.predict_logits(features) # prediction and logits are both 128-dimensional vectors, where prediction is the corresponding classification value for each label

        for k in range(len(prediction)):
            if (prediction[k]!=-1):
                match_list_train = torch.cat((match_list_train, prediction[k].unsqueeze(0)))
                match_list_score.append(logits[k][prediction[k] - 1])
                patch = torch.tensor([(k+1)+(count*128)])
                patch = patch.to(device)
                match_list_test = torch.cat((match_list_test, patch.unsqueeze(0)))
        for i in range(len(images)):
            total_unknown += 1
            if prediction[i] == -1: #Falls into the rejection domain
                correct_unknown += 1
            label_logits.append(np.max(logits[i, :]))
